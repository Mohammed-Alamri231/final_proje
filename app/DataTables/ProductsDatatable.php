...
class ProductsDatatable extends DataTable {
	...
	public function query() {
		// return Product::query()->with('manu_id')->with('department_id')->with('stock_id')->select('products.*');
		if(admin()->user()->type == 'admin'){
		return Product::query()->with('manu_id')->with('department_id')->with('stock_id')->select('products.*');
	}
	elseif (admin()->user()->type == 'company'){
		return Product::query()->where('department_id' , 1)->with('manu_id')->with('department_id')->with('stock_id')->select('products.*');
	}
	}
	/**
	...
	protected function getColumns() {
		return [
            ...
            [
				'name'  => 'serial_number',
				'data'  => 'serial_number',
				'title' => trans('admin.serial_number'),
            ], 
            [
				'name'  => 'price',
				'data'  => 'price',
			...
			, 
            [
				'name'  => 'manu_id.name_'.session('lang'),
				'data'  => 'manu_id.name_'.session('lang'),
				'title' => trans('admin.maun_id'),
			]
			, 
            [
