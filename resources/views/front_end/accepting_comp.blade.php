...
<div class="container">
                ...
                @foreach($member as $members)
                        <form  method="POST" action="{{ url('accept_comp') }}/{{$members->id}}" enctype="multipart/form-data">
                           {{ csrf_field() }}
                          ...
                          <div class="modal-body col-sm-6">
                              ...
                              <div class="form-group">
                                ...
                                <input type="text" name="permit" placeholder="{{ trans('admin.Miedical prment') }}" class="col-md-4" value="{{$members->permit}}">
                          ...
                          <div class="modal-footer">
                            {{-- <button type="button" data-dismiss="modal" class="btn btn-danger">{{ trans('admin.reject') }}</button> --}}
                            <button type="submit" class="btn btn-primary">{{ trans('admin.accept') }}</button>
                            <form method="get" action="{{url('reject')}}/{{ $members->id }}" enctype="multipart/form-data">
                              {{ csrf_field() }}
